...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "the attempt to increment the version failed due to %v", err)
		os.Exit(-4)
	}
	...
	if ver != md.SemVer.String() {
		if err := md.Replace(*verFn, *verFn, false); err != nil {
			fmt.Fprintf(os.Stderr, "the attempt to write the incremented version back failed due to %v", err)
			os.Exit(-4)
		}
	}
