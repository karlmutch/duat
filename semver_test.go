...
//
func createTestFile(version *semver.Version) (fn string, err kv.Error) {
...
func ApplyCase(fileVer *semver.Version, applyVer semver.Version) (err kv.Error) {
	...
	if err != nil {
		return kv.Wrap(err, "failed to apply major version change to version").With("stack", stack.Trace().TrimRuntime()).With("ver", applyVer)
	}
	if result.String() != newMD.SemVer.String() {
		return kv.Wrap(err, "applying the major version change failed to return the same value as that stored").With("stack", stack.Trace().TrimRuntime()).With("ver", applyVer)
	}

	if applyVer.String() != newMD.SemVer.String() {
		return kv.Wrap(err, "applying the major version change failed to return the expected value").With("stack", stack.Trace().TrimRuntime()).With("ver", applyVer)
	}
	return nil
}
...
func TestVersionReplace(t *testing.T) {
	...
	if _, err = originalMD.LoadVer(srcFn); err != nil {
		t.Error(kv.Wrap(err, "failed to read the version from the original file").With("stack", stack.Trace().TrimRuntime()))
		return
	}
	if ver.String() != originalMD.SemVer.String() {
		t.Error(kv.Wrap(err, "the original file was modified unexpectly").With("stack", stack.Trace().TrimRuntime()))
		return
	}
	...
	if _, err = replacedMD.LoadVer(emptyDestFn); err != nil {
		t.Error(kv.Wrap(err, "failed to read the version from the processed file").With("stack", stack.Trace().TrimRuntime()).With("srcFn", srcFn))
		return
	}
	if md.SemVer.String() != replacedMD.SemVer.String() {
		t.Error(kv.Wrap(err, "the processed file was not correctly modified").With("stack", stack.Trace().TrimRuntime()))
		return
	}
	...
	if _, err = minorReplacedMD.LoadVer(minorDestFn); err != nil {
		t.Error(kv.Wrap(err, "failed to read the minor version from the minor version change populated file").With("stack", stack.Trace().TrimRuntime()).With("srcFn", srcFn))
		return
	}
	if mdMinor.SemVer.String() != minorReplacedMD.SemVer.String() {
		t.Error(kv.Wrap(err, "the already populated file was not correctly modified").With("stack", stack.Trace().TrimRuntime()))
		return
	}
}
